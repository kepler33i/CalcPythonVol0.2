...
def TemaClaro(*args):
    ...
    estilos_botones_borrar.configure('Botones_borrar.TButton', background ='#FFFFFF', foreground="black")
...
def ingresarValoresTeclado(event):
    ...
    if tecla == '=' or event.keysym == 'Return':
        entrada1.set(entrada1.get() + entrada2.get())
        resultado = eval(entrada1.get())
        entrada2.set(resultado)
...
button0.grid(column=0, row=6, sticky=(W, N, E, S))
button_punto.grid(column=1, row=6, sticky=(W, N, E, S))
button_resta.grid(column=3, row=6, sticky=(W, N, E, S))
button_potencia.grid(column=2, row=6, sticky=(W, N, E, S))
...
root.bind('<KeyPress-Return>', ingresarValoresTeclado)
root.bind('<Key>', ingresarValoresTeclado)
root.bind('<BackSpace>', borrar)
root.bind('<KeyPress-b>', borrarTodo)
